...
lineStyles = {'-', '--', ':', '-.'};
% Create Figure 3 for the state plots
figure(3);
%hold on;
...
for i = 1:size(gamma_10_in_10)
    ...
    [t, x] = ode45(@(t, x) robotDynamics(t, x, gamma_10_in_10(i), kappa_10_in_10(i)), [0 tfin], xini, optionsODE);
    
    % Plot the results for the current simulation in Figure 3
    %plot(t, x(:, 1), t, x(:, 2), 'DisplayName', ['Gamma: ' num2str(gamma_10_in_10(i)) ', Kappa: ' num2str(kappa_10_in_10(i))]);
    % Plot the results for the current simulation
    plot(t, x(:, 1), 'Color', lineColors(i,:), 'LineStyle', lineStyles{1}, 'LineWidth', 1.5, 'DisplayName', ['Pop: ' num2str(popu) ', \gamma: ' num2str(gamma_10_in_10(i)) ', \kappa: ' num2str(kappa_10_in_10(i))]);
    hold on;
end
...
for i = 1:size(gamma_10_in_10)
    ...
    %plot(t, x(:, 1), t, x(:, 2), 'DisplayName', ['Gamma: ' num2str(gamma_10_in_10(i)) ', Kappa: ' num2str(kappa_10_in_10(i))]);
    % Plot the results for the current simulation
    plot(t, x(:, 2), 'Color', lineColors(i,:), 'LineStyle', lineStyles{1}, 'LineWidth', 1.5, 'DisplayName', ['Pop: ' num2str(popu) ', \gamma: ' num2str(gamma_10_in_10(i)) ', \kappa: ' num2str(kappa_10_in_10(i))]);
    hold on;
end
...
% Create Figure 4 for the state plots
figure(5);
%hold on;
...
%%
function dx = robotDynamics(t, x, gamma, k)
    % Unpack the state variables7
    xg=3; yg=0;
